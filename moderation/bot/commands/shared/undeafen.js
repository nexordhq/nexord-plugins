...
module.exports = async ({ guild, member }, target, reason) => {
    ...
    if (typeof response === "boolean") {
        return guild.getT("moderation:VOICE.UNDEAFEN_SUCCESS", {
    ...
    if (response === "BOT_PERM") {
        return guild.getT("moderation:VOICE.UNDEAFEN_BOT_PERM", {
            target: target.user.username,
        });
    }
    if (response === "NO_VOICE") {
        return guild.getT("moderation:VOICE.UNDEAFEN_NO_VOICE", {
            target: target.user.username,
        });
    }
    ...
    return guild.getT("moderation:VOICE.UNDEAFEN_ERROR", {
        target: target.user.username,
    });
};
